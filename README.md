...
The method I developed to implement simple key authentication to the API consisted of setting up a Lambda function and assigning it to an Authorizer within the HTTP API.

The Lambda function which I named `billing-homeassistant-apikey` was created and the environment was set with the API key in a variable as per the code. The code can be found here: [authorizer-apikey.py](/authorizer-apikey.py).
